...
URL fetches are conditional. After a frame is uploaded, its `ETag` and `Last-Modified` are kept in the system temp dir (`gnpc-http-<name>.json`) and sent back as `If-None-Match` / `If-Modified-Since` on the next fetch; a `304 Not Modified` means glacier.org already shows that frame, so the camera skips its overlays and upload for the round. The record is written only after a successful upload, so a frame that never reached the server is fetched again rather than skipped, and it is ignored if the camera's URL has changed. A source that ignores the headers simply answers 200 every time and behaves as before. A 200 whose body is not an image (nps.gov has served a zero-byte file for hours) is logged as a warning rather than raised: the frame already on glacier.org stays up, and that response's validators are recorded so the bad object answers 304 until the source replaces it. Note that `stmary` and `smv` are different cameras pointed at the same valley from opposite ends — `smv` looks down it from Logan Pass.
